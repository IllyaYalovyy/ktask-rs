...
pub(crate) fn run(project: Option<ktask_core::Project>) -> RunOutcome {
    ...
    match active_task {
        Some(task_id) => {
            ...
            if let Err(e) = control::write_interrupt(&proj) {
                render::progress(format_args!("error writing interrupt signal: {e}"));
                return RunOutcome::Usage {
                    detail: format!("{e}"),
                };
            }

            // Print the task we're interrupting
            let task_title = tasks
                .iter()
                .find(|t| t.id == task_id)
                .map_or("Unknown".to_string(), |t| t.title().to_string());

            render::out(format_args!("id={} title={} action=interrupted", task_id, task_title));
            RunOutcome::Drained
        }
        None => RunOutcome::Usage {
            detail: "no running task to interrupt".to_string(),
        },
    }
}
...
mod tests {
    ...
    #[test]
    fn interrupt_with_no_active_task_exits_2() {
