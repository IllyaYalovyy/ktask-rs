...
enum Remediated {
    /// Remediation ran; continue the task under this attempt id.
    Continue(AttemptId),
    /// Remediation ended the task; return this state.
    Stop(crate::TaskState),
}
...
impl Runner {
    ...
    /// 1. Records attempt start
    /// 2. Runs preflight checks and creates worktree
    /// 3. Executes each phase in the selected protocol
    /// 4. Verifies and publishes on success
    /// 5. Records final state
    ///
    /// Every state transition is journaled before taking effect. The worktree
    /// and repository lock are released on all exit paths (success or failure).
    ///
    /// # Arguments
    ///
    ...
    pub fn run_task(&mut self, task: &Task) -> Result<crate::TaskState> {
        ...
        let worktree_name = format!("task-{}", task.id);
        let _ = crate::git::remove_worktree(&self.project.root, &worktree_name);

        // Clean up is automatic: prepared is dropped here, releasing the lock
        drop(prepared);
    ...
    ///
    /// On failure, attempts remediation if budget allows:
    /// 1. Check remediation budget
    /// 2. Record failure and new attempt
    /// 3. Run the remediation phase
    /// 4. Return new attempt ID or terminal state
    ///
    /// # Returns
    ///
    /// `Remediated::Continue(attempt_id)` if remediation succeeded,
    /// `Remediated::Stop(state)` if remediation budget exhausted or phase failed.
    fn attempt_remediation(
        &mut self,
        task: &Task,
        prepared: &Prepared,
        failing_attempt: AttemptId,
        remediations_used: u32,
        failure_class: FailureClass,
        gate_results: &[crate::GateResult],
    ) -> Result<Remediated> {
        // Budget first: check if we've exhausted the remediation attempts
        if remediations_used >= self.config.max_remediation_attempts {
            let detail = format!(
                "remediation budget exhausted after {} attempts",
                remediations_used
            );
            self.recorder.record(
                Some(task.id),
            ...
            return Ok(Remediated::Stop(crate::TaskState::Failed {
                class: failure_class,
                detail,
            }));
        }

        // Calculate the next attempt ID
        let next = AttemptId::new(failing_attempt.get() + 1);

        // Record VerifyFailed first, then AttemptStarted (state machine requires this order)
        self.recorder.record(
            Some(task.id),
            crate::EventKind::VerifyFailed {
        ...
        // Read prior evidence for the bundle
        ...
        // Run the remediation phase with Implement scope
        let remediation_spec = PhaseSpec {
        ...
        match self.run_phase(prepared, task, next, &remediation_spec) {
            Ok(PhaseOutcome::Success) => Ok(Remediated::Continue(next)),
            Ok(PhaseOutcome::Failure { class, detail }) => {
                ...
                Ok(Remediated::Stop(crate::TaskState::Failed { class, detail }))
            }
            Err(e) => {
    ...
    /// Check the gate for a phase and return the outcome.
    ///
    /// Runs the gate and returns its result without attempting remediation.
    /// The caller decides what to do with gate failure.
    fn gate_outcome(
        &mut self,
        prepared: &Prepared,
        task_id: crate::TaskId,
        ...
        if spec.gate.is_none() {
            return Ok(GateOutcome::NoGate);
        }

        match self.gate_phase(prepared, task_id, attempt, spec, red_phase_summary.as_ref()) {
            Ok(summary) => {
    ...
    /// as phases complete. Called by run_task after successful preparation.
    #[doc(hidden)]
    pub fn run_task_with_prepared(
        &mut self,
        task: &Task,
        attempt: AttemptId,
        prepared: &Prepared,
            ...
            loop {
                ...
                match self.run_phase(prepared, task, attempt, spec) {
                    ...
                    Ok(PhaseOutcome::Failure { class, detail: _ }) => {
                        match self.attempt_remediation(
                            task,
                            prepared,
                            attempt,
                            remediations_used,
                            ...
                            Remediated::Stop(state) => return Ok(state),
                            Remediated::Continue(next) => {
                                attempt = next;
                                remediations_used += 1;
                                continue;
                            }
                ...
                // Gate check for this phase
                match self.gate_outcome(prepared, task.id, attempt, spec, &mut red_phase_summary)? {
                    GateOutcome::Passed | GateOutcome::NoGate => break,
                    GateOutcome::Failed => {
                        match self.attempt_remediation(
                            task,
                            prepared,
...
impl Runner {
    ...
    pub fn run_queue(
        ...
        loop {
            // projection is only maintained by rebuild; see task <invariant task id>
            journal.rebuild_state()?;
...
"#;
    ...
    #[test]
    fn runner_run_task_reaches_done_with_direct_protocol() {
        let repo = ScratchRepo::new().expect("Failed to create scratch repo");
        crate::git::git(repo.path(), &["push", "-u", "origin", "master"])
            .expect("Failed to push to origin");
        ...
        let scenario_file = state_dir.join("scenario.toml");
        // Modify .gitignore (which is already tracked) so there's something to commit
        let scenario = r#"
[[steps]]
outcome = "success"
stdout = "Task completed (Implement phase)"
files = { ".gitignore" = ".ktask/\n# Task output\n" }

[[steps]]
outcome = "success"
stdout = "Task completed (Verify phase)"

[[steps]]
outcome = "success"
stdout = "Task completed (Publish phase)"
"#;
        std::fs::write(&scenario_file, scenario).expect("Failed to write scenario file");
        ...
        config.dummy_scenario_path = Some(scenario_file);
        config.mainline_branch = "master".to_string();
        config.default_protocol = "direct".to_string();
        ...
        // Write the report that the agent would produce
        let report_path = crate::report_path(&project, task.id, attempt_id);
        std::fs::write(&report_path, "KTASK_RESULT: DONE\n").expect("Failed to write report");
        ...
        // Clean up
        let worktree_name = format!("task-{}", task.id);
        let _ = crate::git::remove_worktree(&project.root, &worktree_name);
        drop(prepared);
    ...
    #[test]
    fn runner_run_task_cleans_up_worktree_on_success() {
        let repo = ScratchRepo::new().expect("Failed to create scratch repo");
        crate::git::git(repo.path(), &["push", "-u", "origin", "master"])
            .expect("Failed to push to origin");
...
"#;
        ...
        // Call run_task which should handle cleanup
        let state = runner.run_task(&task).expect("run_task should succeed");

        // Should reach Done state (task is configured to succeed)
        let _ = state;

        // Check that worktree is cleaned up
    ...
    #[test]
    fn runner_run_task_cleans_up_on_preflight_failure() {
        let repo = ScratchRepo::new().expect("Failed to create scratch repo");
        ...
        let mut config = Config::default();
        config.baseline_command = Some(vec!["false".to_string()]); // Will fail
        config.verify_command = Some(vec!["true".to_string()]);
        config.dummy_scenario_path = Some(scenario_file);
...
"#;
    ...
    mod outcome {
        ...
        #[test]
        fn provider_limit_without_time() {
